...
void initial(vector<vector<double> >& a, vector<double>& b, int n) {
    ...
    cout << "Setting interior points with consistent 6th-order 9 points difference scheme ..." << endl;
    ...
    cout << "for 9 points difference scheme(2-order bound)  ,Interior  points: " << (NX-4)*(NY-4) << endl;
}
...
void output(int m) {
    ...
    ostringstream name;
    name << "9 points difference scheme(2-order bound) " << NX << "x" << NY << "_" << setfill('0') << setw(6) << m << ".vtk";
    ofstream out(name.str().c_str());
    
    out << "# vtk DataFile Version 3.0\n";
    out << "9 points difference scheme(2-order bound)_\n";
    out << "ASCII\n";
...
void output_gnuplot_data() {
    ...
    ofstream data_file("grid_convergence_9 points difference scheme(2-order bound).dat");
    ...
    data_file.close();
    cout << "Data file output: grid_convergence_9 points difference scheme(2-order bound).dat" << endl;
    ...
    ofstream gnuplot_file("plot_convergence_9 points difference scheme(2-order bound).plt");
    gnuplot_file << "set terminal png enhanced size 800,600" << endl;
    gnuplot_file << "set output 'grid_convergence_9 points difference scheme(2-order bound).png'" << endl;
    ...
    gnuplot_file << "plot 'grid_convergence_9 points difference scheme(2-order bound).dat' using 3:5 with linespoints pt 7 ps 1.5 lw 2 title sprintf('Improved (slope = %.2f)', " << slope << "), \\" << endl;
    gnuplot_file << "     f(x) with lines lw 2 lc rgb 'red' title sprintf('Linear Fit (slope = %.2f)', " << slope << "), \\" << endl;
    gnuplot_file << "     g(x) with lines lw 2 lc rgb 'green' dashtype 2 title '9 points difference scheme(2-order bound)(slope = 4.0)'" << endl;

    gnuplot_file.close();
    cout << "Gnuplot script output: plot_convergence_9 points difference scheme(2-order bound).plt" << endl;
...
int main() {
    for(int grid_idx = 0; grid_idx < Nx.size(); grid_idx++) {
        ...
        cout << "Program execution started with  9 points difference scheme(2-order bound) ...." << endl;
        steadystate = false;
        initial(a, b, n);  //初始化
