...
double L1sum;
double maxerror;
const double tolerance = 1.422e-14; // 迭代收斂判據
bool steadystate;
...
void initial(vector<vector<double> >& a, vector<double>& b, int n) {
	...
	for(int j = 3 ; j <= NY-2 ; j++){
		...
		a[idx][idx-NX] = a2*H ;//(4)下
		a[idx][idx+1+NX] = a3*H ;//(5)右上
		a[idx][idx-1+NX] = 0.0 ;//(6)左上
		a[idx][idx-1-NX] = 0.0;//(7)左下
		a[idx][idx+1-NX] = a3*H ;//(8)右下 
        b[idx] = -a2*H*T_left -a3*H*T_left -a3*H*T_left ;
	} 
	//右邊界計算點 
	//處理方式:東西方向採用單邊插分，南北方向採用中心差分
	for(int j = 3 ; j <= NY-2 ; j++){
		...
		a[idx][idx-1] = a2*H ;//(3)左 
		a[idx][idx-NX] = a2*H ;//(4)下
		a[idx][idx+1+NX] = 0.0 ;//(5)右上
		a[idx][idx-1+NX] = a3*H ;//(6)左上
		a[idx][idx-1-NX] = a3*H ;//(7)左下
		a[idx][idx+1-NX] = 0.0 ;//(8)右下 
        b[idx] = -a2*H*T_right -a3*H*T_right -a3*H*T_right ;
	} 
    //下邊界計算點
	...
	for(int i = 3 ; i <= NX-2 ; i++){
        ...
        a[idx][idx+NX] = a2*H ;//(2)上
		a[idx][idx-1] = a2*H ;//(3)左 
		a[idx][idx-NX] = 0.0 ;//(4)下
		a[idx][idx+1+NX] = a3*H ;//(5)右上
		a[idx][idx-1+NX] = a3*H ;//(6)左上
		a[idx][idx-1-NX] = 0.0 ;//(7)左下
		a[idx][idx+1-NX] = 0.0  ;//(8)右下 
        b[idx] = -a2*H*T_bottom -a3*H*T_bottom -a3*H*T_bottom ;
	} 
	//上邊界計算點
	//處理方式:南北方向採用單邊差分，東西方向採用中心差分
	for(int i = 3 ; i <= NX-2 ; i++){
	    ...
	    a[idx][idx-1-NX] = a3*H ;//(7)左下
		a[idx][idx+1-NX] = a3*H ;//(8)右下 
        b[idx] = -a2*H*T_up(i) -a3*H*T_up(i-1) -a3*H*T_up(i+1) ;
	}
	...
	a[p1][p1-1] = 0.0  ;//(3)左 
	a[p1][p1-NX] = 0.0 ;//(4)下
	a[p1][p1+1+NX] = a3*H ;//(5)右上
	a[p1][p1-1+NX] = 0.0 ;//(6)左上
	a[p1][p1-1-NX] = 0.0  ;//(7)左下
	a[p1][p1+1-NX] = 0.0 ;//(8)右下 
    b[p1] = -a2*H*T_left - a2*H*T_bottom - a3*H*T_left - a3*H*T_left - a3*H*T_bottom ;
    //右下角點
	int p2 = (2-1)*NX + (NX-1) ; 
	a[p2][p2] = a1*H ;  
    a[p2][p2+1] = 0.0 ;//(1)右 
	...
	a[p2][p2-1] = a2*H ;//(3)左 
	a[p2][p2-NX] = 0.0 ;//(4)下
	a[p2][p2+1+NX] = 0.0 ;//(5)右上
	a[p2][p2-1+NX] = a3*H ;//(6)左上
	a[p2][p2-1-NX] = 0.0 ;//(7)左下
	a[p2][p2+1-NX] = 0.0 ;//(8)右下 
    b[p2] = -a2*H*T_right - a2*H*T_bottom - a3*H*T_right - a3*H*T_right - a3*H*T_bottom ;
	//左上角點
	int p3 = (NY-2)*NX + 2;
    a[p3][p3] = a1*H ;  
    a[p3][p3+1] = a2*H ;//(1)右 
	...
	a[p3][p3-1] = 0.0 ;//(3)左 
	a[p3][p3-NX] = a2*H ;//(4)下
	a[p3][p3+1+NX] = 0.0;//(5)右上
	a[p3][p3-1+NX] = 0.0 ;//(6)左上
	a[p3][p3-1-NX] = 0.0 ;//(7)左下
	a[p3][p3+1-NX] = a3*H ;//(8)右下 
    b[p3] = -a2*H*T_left - a2*H*T_up(2) - a3*H*T_left - a3*H*T_up(1) - a3*H*T_up(3) ;
    //右上角點
	int p4 = (NY-2)*NX + (NX-1);
    a[p4][p4] = a1*H ;  
    a[p4][p4-1] = 0.0 ;//(1)右 
	...
	a[p4][p4-1] = a2*H ;//(3)左 
	a[p4][p4-NX] = a2*H ;//(4)下
	a[p4][p4+1+NX] = 0.0 ;//(5)右上
	a[p4][p4-1+NX] = 0.0 ;//(6)左上
	a[p4][p4-1-NX] = a3*H ;//(7)左下
	a[p4][p4+1-NX] = 0.0 ;//(8)右下 
    b[p4] = -a2*H*T_right - a2*H*T_up(NX-1) - a3*H*T_right - a3*H*T_up(NX) - a3*H*T_up(NX-2) ;
...
void SOR(vector<vector<double> >& a, vector<double>& b, vector<double>& x, int n) {
    ...
    if(G > 10000) omega = 2.0 ; // 大型矩陣使用較大鬆弛因子
    //保存舊的解
    for(int k = 1; k <= n; k++) {
        x_old[k] = x[k];
    }
...
void output(int m) {
    ...
    ostringstream name;
    name << "9 points difference shceme(6-order bound)_" << NX << "x" << NY << "_" << setfill('0') << setw(6) << m << ".vtk";
    ofstream out(name.str().c_str());
    
    out << "# vtk DataFile Version 3.0\n";
    out << "9 points difference shceme(6-order bound)_\n";
    out << "ASCII\n";
...
void output_gnuplot_data() {
    ...
    ofstream data_file("grid_convergence_9 points difference shceme(6-order bound).dat");
    ...
    data_file.close();
    cout << "Data file output: grid_convergence_9 points difference shceme(6-order bound).dat" << endl;
    ...
    ofstream gnuplot_file("plot_convergence_9 points difference shceme(6-order bound).plt");
    gnuplot_file << "set terminal png enhanced size 800,600" << endl;
    gnuplot_file << "set output 'grid_convergence_9 points difference shceme(6-order bound).png'" << endl;
    gnuplot_file << "set title '9 points difference scheme (6-order bound)'" << endl;
    ...
    gnuplot_file << "g(x) = 6.0 * (x - " << x_ref << ") + " << y_ref << endl;
    
    gnuplot_file << "plot 'grid_convergence_9 points difference shceme(6-order bound).dat' using 3:5 with linespoints pt 7 ps 1.5 lw 2 title sprintf('Improved (slope = %.2f)', " << slope << "), \\" << endl;
    gnuplot_file << "     f(x) with lines lw 2 lc rgb 'red' title sprintf('Linear Fit (slope = %.2f)', " << slope << "), \\" << endl;
    gnuplot_file << "     g(x) with lines lw 2 lc rgb 'green' dashtype 2 title '9 points difference shceme(6-order bound)(slope = 6.0)'" << endl;

    gnuplot_file.close();
    cout << "Gnuplot script output: plot_convergence_9 points difference shceme(6-order bound).plt" << endl;  
...
void output_gnuplot_data2() {
    ...
    double slope = (n_points * sum_xy - sum_x * sum_y) / (n_points * sum_x2 - sum_x * sum_x);
    double intercept = (sum_y - slope * sum_x) / n_points;
    ofstream gnuplot_file("plot_convergence_9 points difference shceme(6-order bound)2.plt");
    gnuplot_file << "set terminal png enhanced size 800,600" << endl;
    gnuplot_file << "set output 'grid_convergence_9 points difference shceme(6-order bound)2.png'" << endl;
    gnuplot_file << "set title '9 points difference scheme (6-order bound)2'" << endl;
    gnuplot_file << "set xlabel 'log(dx)'" << endl;
    gnuplot_file << "set ylabel 'log(L1 Error)'" << endl;
    gnuplot_file << "set grid" << endl;
    gnuplot_file << "set key left top" << endl;
    ...
    gnuplot_file.close();
    cout << "Gnuplot script output: plot_convergence_9 points difference shceme(6-order bound)2.plt" << endl;
